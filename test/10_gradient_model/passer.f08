...
program caller
    ...
    ! Setup Variables
    !
    integer*NATMKIND, parameter :: natm = 9
    real*8 :: coords(3,natm)
    integer*ANUMKIND :: atmnum(natm)
    integer(c_int) :: port = 8504 
    character(len=*), parameter :: model_name = "gradientmodel"
    ...
    call stage_calculation(natm, coords, atmnum, tfs)

end program caller
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine stage_calculation(natm, coords, atmnum, tfs)
    ...
    integer :: max_atoms, max_bas, num_els
    ...
    real(c_double), dimension(MAXBAS, MAXATOM, NUMELS) :: rad_bas, ang_bas, gradient
    ...
    integer :: num_rads(2) = [RAD, RAD], &
               ...
               num_bas(2) = [RAD + HANG, RAD + OANG], i, j, k
    max_atoms = MAXATOM
    max_bas = MAXBAS
    num_els = NUMELS
    energy = 0d0
    ...
    do  el=1, num_els
       rad_bas(num_rads(1)+1:num_rads(el)+num_angs(el),:MAXATOM,el) = ang_bas(:num_angs(el),:MAXATOM,el) 
    enddo

    ! Send the concated basis to the server
    call tfs%gradBPSF(rad_bas, max_bas, max_atoms, num_bas, g_num_of_els, NUMELS, energy, gradient)
    do i=1, NUMELS
        do j=1, MAXATOM
            do k=1, MAXBAS
            if (gradient(k,j,i) .ne. 0) &
                print*, "Basis el, atm, bas is: ", i, j, k, gradient(k,j,i)
            enddo
        enddo
    enddo 
    call print_basis(gradient, MAXBAS, MAXATOM, num_rads, g_num_of_els, NUMELS)
