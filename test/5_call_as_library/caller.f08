...
subroutine write_basis(rad_bas, ang_bas, rad_grad, ang_grad)
  ...
  CALL h5dcreate_f(file_id, "h_radial_sym_funcs", H5T_NATIVE_DOUBLE, dspace_id, &
       dset_id, error)
  CALL h5dwrite_f(dset_id, H5T_NATIVE_double, rad_bas(:RAD,:NUMH,1), hrb_dims, error)
  ...
  CALL h5dcreate_f(file_id, "h_rad_cartesian_gradient", H5T_NATIVE_DOUBLE, dspace_id, &
       dset_id, error)
  CALL h5dwrite_f(dset_id, H5T_NATIVE_double, rad_grad(:,:MAXATOM,:HANG,:NUMH,1), hrg_dims, error)
  CALL h5dclose_f(dset_id , error)
  CALL h5sclose_f(dspace_id, error)
  ! *************** O RAD GRAD *********************
  ...
  CALL h5dcreate_f(file_id, "o_rad_cartesian_gradient", H5T_NATIVE_DOUBLE, dspace_id, &
       dset_id, error)
  CALL h5dwrite_f(dset_id, H5T_NATIVE_double, rad_grad(:,:MAXATOM,:OANG,:NUMO,2), org_dims, error)
  CALL h5dclose_f(dset_id , error)
  CALL h5sclose_f(dspace_id, error)
  ! *************** H ANG BASIS ********************
