...
\end{equation}
\begin{equation}
\frac{d R_{ij}}{d \mb{X}_{j}} = - \frac{\partial R_{ij}}{\mb{X}_{i}}
\end{equation}

The gradient of the smoothing function with respect to a cartesian coordinate:
\begin{equation}
\frac{\partial f_c (R_{ij}) } { \mb{X}} = -\frac{\pi}{2 
 \mathrm{R_s}}  \sin ( \frac{\pi R_{ij}}{\mathrm{R_s}} + 1 ) \
\end{equation}

The gradient of the $j$-th component of the $i$-th radial symmetry function:
\begin{equation}
\frac{\partial G_{i,j}^2 } { \mb{X}} e^{-\eta ( R_{ij} - \mathrm{R_s} )^2} \left[ -2 \eta ( R_{ij} - \mathrm{R_s} )  f_c (R_{ij}) \frac{d R_{ij}}{d R_{ij}} + \frac{d f_c ( R_{ij} )}{d R_{ij}} \right] 
\end{equation}
